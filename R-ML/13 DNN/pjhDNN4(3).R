...
for (pSeed in pSeedList) {
  ...
  for (Rate in Rates)  {
    ...
    accuTes <- dnnH3 %>% evaluate(x_Tes, y_Tes, verbose=0)
    Accu.tra <- round(accuTra[[2]] * 100, digits =2)
    Accu.val <- round(accuVal[[2]] * 100, digits =2)
    Accu.tes <- round(accuTes[[2]] * 100, digits =2)
    cat(sprintf('\n Rate = %6.2f%% seed = %5d Train : %6.2f%% Valid : %6.2f%% Tes: %6.2f%%',
                Rate, pSeed, Accu.tra, Accu.val,Accu.tes))
